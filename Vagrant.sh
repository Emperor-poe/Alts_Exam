...
sudo chmod -R 755 /var/www/html/app\" > vagrant.sh"

echo "
in_slave_ip=\$(ip addr show enp0s8 | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | tr -d '\r')" >> vagrant.sh
...
cat vagrant.sh | vagrant ssh Master -c "cat >> vagrant.sh"
...
sudo service apache2 restart\" >> vagrant.sh"

vagrant ssh Master -c "chmod 777 vagrant.sh"

#Generate ssh key for master node as altschool user
